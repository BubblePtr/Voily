...
public enum TranslationOutputStyle: Equatable, Sendable {
    case natural
}

public enum TextProcessingMode: Equatable, Sendable {
...
public struct TextProcessingRequest: Equatable, Sendable {
    ...
    public let mode: TextProcessingMode

    public init(text: String, languageCode: String, mode: TextProcessingMode) {
        self.text = text
...
public final class LLMRefinementService {
    enum LLMError: Error {
        case invalidBaseURL
    ...
    @MainActor
    public func process(_ request: TextProcessingRequest, settings: AppSettings) async throws -> String {
    ...
    @MainActor
    public func testConnection(settings: AppSettings) async throws {
        _ = try await process(
    ...
    private static func dictationPrompt(
        ...
        - 不要删除任何看起来已经正确的内容。
        - 如果输入主要是英文，默认保留英文表达，不要翻成中文。
        - 只有语义明显不通或疑似识别错词时，才按上下文修正；最终使用最符合原意的中文或英文。
